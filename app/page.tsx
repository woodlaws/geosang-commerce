...
import { commonFaqs, team } from "@/data/site";
  ...
  return (
    <main>
      ...
      <section className="benefit-strip shell section-gap">
        {[ ["↗", "실제 공급 가능한 상품", "정확한 공동구매 가격과 구성을 확인하고 제안받을 수 있습니다."], ["□", "재고 부담 없는 구조", "공급사가 상품 재고와 소비자 배송을 담당합니다."], ["◇", "공급·배송·운영 지원", "거상마케팅센터 운영팀이 일정과 캠페인 진행을 함께합니다."] ].map(([icon,title,desc]) => <article key={title}><span>{icon}</span><h3>{title}</h3><p>{desc}</p></article>)}
      </section>
      ...
      <section className="shell faq-wrap section-gap"><div><span className="eyebrow">FAQ</span><h2>자주 묻는 질문</h2><p>공동구매 지원과 운영 과정에서 자주 묻는 내용을 확인하세요.</p><Link href="/faq" className="outline-button compact">전체 질문 보기</Link></div><FAQAccordion items={commonFaqs} /></section>
    </main>
  );
}
