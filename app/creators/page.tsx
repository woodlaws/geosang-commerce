...
const process = [["01", "크리에이터 파트너 등록", "기본 정보와 활동 채널, 관심 분야를 등록합니다."], ["02", "채널과 콘텐츠 확인", "콘텐츠 분야와 팔로워 반응, 활동 이력을 확인합니다."], ["03", "상품 매칭과 조건 협의", "적합한 상품의 구성, 일정, 수익과 정산 조건을 협의합니다."], ["04", "제품 체험과 콘텐츠 기획", "상품을 이해하고 채널에 맞는 콘텐츠 방향을 정합니다."], ["05", "콘텐츠 제작과 확인", "상품 정보와 광고 표시 기준을 확인해 콘텐츠를 준비합니다."], ["06", "공동구매 오픈과 판매", "확정된 일정과 판매 링크로 팔로워에게 상품을 소개합니다."], ["07", "판매 마감과 정산", "취소·반품 내역을 반영한 뒤 합의한 기준으로 정산합니다."]];
...
export default function CreatorsPage() {
  ...
  return <main className="creators-page"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(structuredData) }} />
    <section className="creator-hero"><div className="shell creator-hero-inner"><div><span className="eyebrow">CREATOR PARTNERS</span><h1>좋은 콘텐츠가<br /><em>좋은 제품을 만나는 순간</em></h1><p>거상커머스는 상품과 콘텐츠의 적합성을 중심으로 인플루언서를 매칭합니다. 상품 공급, 배송 협의, 캠페인 운영과 정산 안내를 지원하므로 크리에이터는 콘텐츠와 팔로워 소통에 집중할 수 있습니다.</p><strong>팔로워 수보다 콘텐츠의 신뢰와 상품 적합성을 봅니다.</strong><div className="creator-hero-actions"><Link href="#apply" className="gradient-button">인플루언서 파트너 등록</Link></div><div className="creator-microcopy">{["공동구매 경험이 없어도 등록 가능", "재고 부담 없는 협업 방식 우선", "인스타그램·틱톡·유튜브·쇼츠·블로그 지원", "적합한 상품이 있을 때 개별 제안"].map((item) => <span key={item}>✓ {item}</span>)}</div></div><div className="creator-hero-panel"><span>CREATOR COMMERCE</span><h2>거상커머스<br />크리에이터 파트너 등록</h2><p>좋은 제품을 내 콘텐츠로 소개하고, 영향력을 판매 기회로 연결해 보세요.</p>{["상품과 채널 적합성 검토", "협업 조건 사전 협의", "콘텐츠·운영 정보 지원"].map((item, i) => <div key={item}><b>0{i + 1}</b><strong>{item}</strong></div>)}</div></div></section>
