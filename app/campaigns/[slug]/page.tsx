...
import { ManukaCampaignDetail } from "@/components/ManukaCampaignDetail";
import { PropolisCampaignDetail } from "@/components/PropolisCampaignDetail";
import { QuokkiesCampaignDetail } from "@/components/QuokkiesCampaignDetail";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const url = absoluteUrl(`/campaigns/${slug}`);
  const image = item.image ? absoluteUrl(item.image) : undefined;
  return { title: item.seoTitle ? { absolute: item.seoTitle } : item.name, description, alternates: { canonical: url }, openGraph: { title, description, url, siteName: siteConfig.name, locale:"ko_KR", type:"website", images: image ? [{ url:image, alt:item.imageAlt }] : [] }, twitter: { card:"summary_large_image", title, description, images: image ? [image] : [] } };
}
export default async function CampaignDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const faq = item.quokkiesDetail?.faqs || item.propolisDetail?.faqs || item.detail?.faqs || [
  ...
  const jsonLd = { "@context":"https://schema.org", "@graph":[
    ...
    { "@type":"Product", "@id":`${campaignUrl}#product`, name:item.quokkiesDetail?.displayName || item.name, brand:{"@type":"Brand",name:item.brand}, manufacturer:(item.propolisDetail?.productFacts || item.detail?.productFacts)?.find((fact)=>fact.label === "제조사")?.value ? {"@type":"Organization",name:(item.propolisDetail?.productFacts || item.detail?.productFacts)!.find((fact)=>fact.label === "제조사")!.value} : undefined, category:item.category, countryOfOrigin:item.origin?{"@type":"Country",name:item.origin}:undefined, weight:item.quokkiesDetail?{"@type":"QuantitativeValue",value:120,unitCode:"GRM"}:undefined, description:item.seoDescription||`${item.brand} ${item.name} 공동구매 캠페인`, image:item.image?[absoluteUrl(item.image)]:undefined, url:campaignUrl, offers:item.variants.map((variant)=>({"@type":"Offer",name:variant.composition,price:variant.offerPrice,priceCurrency:"KRW",url:campaignUrl,...(item.detail || item.propolisDetail || item.quokkiesDetail || item.status === "준비 중" ? {} : { availability:"https://schema.org/InStock" })})) },
  ...
  if (item.quokkiesDetail) return <><CampaignAnalytics slug={item.slug} name={item.name}/><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} /><QuokkiesCampaignDetail item={item}/></>;
  if (item.propolisDetail) return <><CampaignAnalytics slug={item.slug} name={item.name}/><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} /><PropolisCampaignDetail item={item}/></>;
  if (item.detail) return <><CampaignAnalytics slug={item.slug} name={item.name}/><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} /><ManukaCampaignDetail item={item}/></>;
  ...
  <section className="soft-section section-gap"><div className="shell faq-wrap"><div><span className="eyebrow">FAQ</span><h2>자주 묻는 질문</h2></div><FAQAccordion items={faq}/></div></section><div className="sticky-detail-cta"><span><strong>{item.name}</strong><small>{item.shipping} · 제안가 {won(lead.offerPrice)}부터</small></span><TrackedLink href={`/creators?campaign=${item.slug}#apply`} analyticsEvent="click_creator_apply" analyticsData={{campaign_slug:item.slug}} className="gradient-button">이 상품 제안받기</TrackedLink></div></main>;
}
