...
export function CampaignCard({ campaign }: { campaign: Campaign }) {
  ...
  return <article className="campaign-card">
    <div className="campaign-feed-head"><span className="campaign-feed-avatar">G</span><div><strong>{campaign.brand.toLowerCase()}</strong><small>거상커머스 · {campaign.origin || campaign.category}</small></div><b aria-hidden="true">•••</b></div>
    ...
    <div className="campaign-body"><div className="campaign-social-actions" aria-hidden="true"><span>♡</span><span>◯</span><span>⌁</span><b>▢</b></div><span className="brand-label">{campaign.brand}</span><h3>{campaign.name}</h3>{campaign.origin && <p className="campaign-meta">원산지 {campaign.origin} · {campaign.flavors?.map((flavor) => flavor.name).join(" / ")}</p>}<p className="composition">대표 구성 · {lead.composition}</p>
